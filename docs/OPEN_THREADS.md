...
| A18 | **RUNNING — DR3 Method 2, implicit-diff attribution.** dh\*/de = (I−J)⁻¹∂F/∂e at the fixed point via adjoint Neumann (1 VJP/term) + 1 VJP through the adapter. Ground truth is the marker token; a RANDOM readout direction is the null. **Truncation adaptive, correcting DR3's fixed ~30–40 JVPs**: our rates need 33/38, and 75 for D127's rotating templates — a fixed cut would bias by template type. | running | exact input attribution without BPTT, free of the ρ^(R−r) artefact | `running` |
