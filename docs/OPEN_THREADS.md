...
| A15 | **Reconcile `directions.md`'s status column against the ledger.** Two threads were listed unrun today while their results were banked: B3 (closed by `ds_eigen`, D119) and B4.8 (closed by D64, D123). Both were filed under RUN names, not thread names. | ~30 min | stops a third instance, and stops GPU being spent on answered questions | `open` |
