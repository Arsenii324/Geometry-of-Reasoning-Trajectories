...
| A2 | **G2.3 — recompute the key statistics as GEODESIC/tangent-space quantities.** D99 fixed ‖h‖ to 4 s.f., so PR, step cosine, winding and inter-vertex distance are all chord approximations on a sphere. Candidate explanation for why every rotation instrument failed (D28, D74(6), D83): rotation on a sphere must be measured in the tangent space. | ~1 h on banked orbits | whether the H2 nulls are an artefact of ambient-space coordinates | `open` |
