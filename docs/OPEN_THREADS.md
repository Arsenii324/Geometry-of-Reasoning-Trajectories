...
| B1b | **Redo the state arm properly**: inject ONCE at r rather than overwriting every unroll, and capture the donor's state at the MATCHING unroll. Only this gives the e-vs-state contrast D108 was designed to make. | ~30 min GPU | whether state and parameter interventions really behave oppositely in r | `open` |
