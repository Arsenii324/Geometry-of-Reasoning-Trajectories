...
| A11 | **Does the damped period-6 mode (D112) exist on other prompt families, and does its ARGUMENT vary?** D112 pins λ = 0.911·e^(±iπ/3) on `lenmatch`; `geomcap` and `b6bank` orbits are banked and untested for the argument specifically. If the rotation angle is family-dependent too, the spectrum is a task-level object. | ~30 min, banked | whether D115's family-level rate extends to the full complex eigenvalue | `open` |
