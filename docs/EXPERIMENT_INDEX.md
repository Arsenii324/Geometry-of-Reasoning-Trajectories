...
| **D30** | GATE PASSED: the post-convergence residual IS arithmetic — confirmed on real GPU across three dtypes. | `kaggle_precision_gate` | — | — | kaggle_precision_gate: 1 |
| **D31** | ρ(∂ₕR) measured EXACTLY on real Huginn for the first time — the quantity A2/D21 record as never measured. | `kaggle_jacobian` | — | — | kaggle_jacobian: 1 |
| **D32** | Barannikov's Task a and Task b, run: the register content IS linearly decodable from the latents for both — but the winding readout shows NO quantisat | `kaggle_register` | — | — | kaggle_register: 1 |
| **D33** | H2 restated behaviourally: required recurrent depth DOES scale with difficulty (rho=+0.626, p=0.0014) — but on a length-confounded task, exactly as pr | `kaggle_depth` | — | `run_depth_threshold.py` | kaggle_depth: 1 |
| **D34** 🔻 | SUPERSEDED BY D50 (2026-08-04): the direction is the current-token contrast and registerr is a window artefact. | — | — | — | — |
| **D35** | SCOPED BY D54 (2026-08-04): the effect is in the answer process, not in when the count becomes decodable; and the untrained control that deflated D40/ | `kaggle_depth_fixed` | — | — | kaggle_depth_fixed: 1 |
...
| **D37** | A sharp dissociation: the counting register's CONTENT is fully present after ONE unroll and does not grow with depth, while its DIRECTION rotates mono | `kaggle_register_depth` | — | — | kaggle_register_depth: 27 |
| **D38** | What the extra depth is FOR: readout. | `kaggle_readout` | — | — | kaggle_readout: 5 |
| **D39** | "Geometry encodes the computation" is TRUE — at the answer token, across prompts. | `kaggle_readout` | — | — | kaggle_readout: 5 |
| **D40** | THE CONTROL THAT SHOULD HAVE BEEN RUN FIRST: an UNTRAINED model with random weights reproduces the project's positive findings, most of them MORE stro | `kaggle_baseline` | — | — | kaggle_baseline: 2 |
| **D41** 🔻 | Decodability and capability move in OPPOSITE directions: the untrained model decodes the count 20× more precisely than the trained one and cannot coun | `kaggle_rho_direct` | — | — | kaggle_rho_direct: 2 |
...
| **D44** 🔻 | D42 CONFIRMED ON THE OPERATOR: the untrained map contracts at ρ=0.7150±0.0069, the trained at ρ=0.8866±0.0362. | — | — | — | — |
| **D45** 🔻 | HEADLINE RETRACTED 2026-08-04, same day, after adversarial review. | — | — | — | — |
| **D46** 🔻 | D40 CONFIRMED at matched n=220, and the omitted control comes out AGAINST the deflationary reading: the register direction is NOT the tokenizer's embe | `kaggle_baseline` | — | — | kaggle_baseline: 2 |
| **D47** | The causal patching experiment is BROKEN, not null — the intervention never reached the output. | `kaggle_causal` | — | — | kaggle_causal: 2 |
...
| **D51** 🔻 | SUPERSEDED BY D52 (same day): partial, 4 of 8 checkpoints and n=1 untrained. | — | — | — | — |
| **D52** 🔻 | D44's pseudoreplication is CLOSED on both sides. | — | — | — | — |
| **D53** | THE COUNTING REGISTER IS ARCHITECTURAL. | — | — | `recheck_register_window.py` | — |
...
| **D57** | The Caesar screen is 0% in every cell — but the trained model answers almost every prompt with the memorised pangram "The quick brown fox jumps over t | — | — | — | — |
| **D58** | The log-linear fit used for every ρ in this project is biased +0.033 at Huginn's own ρ and rotation period — an order of magnitude worse than the +0.0 | `kaggle_eps_sweep` | — | — | kaggle_eps_sweep: 2 |
| **D59** | ρ IS STEERABLE by a gradient-free static rescale — paired slope +0.2853 ± 0.0450, r=+0.913, p=2.3e-04, 9/12 prompts monotone. | — | — | — | — |
| **D60** | Prompt format matters enormously — a trivial copy task goes 15% → 100% under Huginn's own chat template — but it does NOT rescue counting or Caesar, w | — | — | — | — |
| **D61** | When Huginn cannot do a task it does not fail randomly — it RETRIEVES something adjacent, by one of at least seven distinct routes, and which route fi | — | — | — | — |
| **D62** | The task-accuracy run is INVALID: batchedgenerate returned an empty string for every prompt. | `kaggle_taskacc` | — | — | kaggle_taskacc: 2 |
| **D63** | The attn-vs-mlp split is UNDERPOWERED and its own pre-registered gate says do not interpret it: the reproduction arm missed D59 by 0.095, larger than  | — | — | — | — |
...
| **D66** | The activation budget was never the problem, and continuouscompute CANNOT WORK in the uncached loop. | `kaggle_memprobe` | — | — | kaggle_memprobe: 2 |
...
| **D68** | THE MODEL KNOWS ANSWERS IT NEVER EMITS, AND DEPTH CONVERGES TO A DISCOURSE CHOICE. | `kaggle_graded` | — | — | kaggle_graded: 2 |
| **D69** 🔻 | (A) DISCOURSE CONFIRMED, BUT BY THE ARM I DID NOT NOMINATE. | `kaggle_discourse` | — | — | kaggle_discourse: 2 |
| **D70** | 6.1's CONFOUND IS NOT SUPPORTED: the content gap is 0 even where the trained model is 100% capable — but the content probe is AT CEILING in both arms, | `kaggle_capgraded` | — | — | kaggle_capgraded: 2 |
| **D71** | THE READOUT UNDER D68/D69 IS EXACT: my per-unroll decode reproduces the model's own logits to max | `kaggle_headcheck` | — | — | kaggle_headcheck: 2 |
| **D72** 🔻 | B6 IS VOID AS DESIGNED: four cells cleared Bonferroni and NONE can be read, because correctness is very nearly a function of the ANSWER VALUE. | `kaggle_geomcorrect` | — | — | kaggle_geomcorrect: 2 |
| **D73** | THE CONTENT NULLS WERE A TARGET-SELECTION ARTIFACT. | `kaggle_nonlinear` | — | — | kaggle_nonlinear: 2 |
...
| **D75** | THE MODEL DOES FAR MORE THAN THIS PROJECT THOUGHT, AND B6 IS UNBLOCKED. | `kaggle_battery` | — | `run_battery.py` | kaggle_battery: 2 |
...
| **D77** | HOW FAR bf16 STORAGE SUPPORTS TRAJECTORY GEOMETRY, MEASURED RATHER THAN ASSUMED: exactly to the unroll where the step norm crosses the rounding scale, | — | — | — | — |
| **D78** | THE PER-UNROLL RANK READOUT HAS AN UNCONTROLLED SOURCE OF RUN-TO-RUN VARIANCE: initializestate draws h0 from an UNSEEDED RNG at a shape that depends o | — | — | — | — |
| **D79** | B6 ANSWERED, AND THE ANSWER IS A BOUNDED NULL: at matched answer value, correct and incorrect trajectories do NOT differ in effective dimensionality,  | — | — | `run_b6.py` | — |
| **D80** | TRAJECTORY SHAPE IS MOSTLY A CLOCK: the geometry statistics are governed by HOW FAR THE CONTRACTION HAS RUN, not by the input. | — | — | `run_window_law.py` | — |
...
| **D83** | H2 IS NOT SUPPORTED BY ROTATION EITHER: the paired track-minus-local difference in rotation per unroll does not scale with reasoning length (rho = +0. | `kaggle_h2rot` | — | `run_h2_rotation.py` | kaggle_h2rot: 2 |
...
| **D85** | NO GEOMETRIC STATISTIC TRACKS CAPABILITY ACROSS 21 TASK FAMILIES SPANNING 0% TO 100% -- AT ANY OF FIVE WINDOWS -- WHILE THE SAME STATISTICS ON THE SAM | — | — | `run_geomcap.py` | — |
| **D86** | DEPTH DOES NOT DESTROY THE ANSWER, IT WRAPS IT IN PROSE -- measured on DECODED STRINGS across 21 families, not on rank. | — | — | `run_depth_accuracy.py` | — |
| **D87** | THE ORBIT'S SHAPE SEPARATES TWO COMPUTATIONS AT AN IDENTICAL TOKEN COUNT, at 96.9-100% balanced accuracy. | `kaggle_lenmatch` | — | `run_shape_decode.py` | kaggle_lenmatch: 190 |
| **D88** | THE SHAPE READS THE INPUT TOKEN, NOT THE COMPUTATION -- the control I expected to confirm the opposite. | `kaggle_marker` | — | `run_shape_decode.py` | kaggle_marker: 218 |
| **D89** | A MEASUREMENT DEFECT IN THE CAPABILITY AXIS ITSELF: correct scores the FIRST TOKEN of gold, and 8 of 21 families have multi-token golds -- 4 of them f | `kaggle_geomcap` | — | — | kaggle_geomcap: 610 |
| **D90** | WHETHER HUGINN ANSWERS A FIXED PROMPT CORRECTLY IS PARTLY DECIDED BY ITS OWN RANDOM INITIAL LATENT. | `kaggle_geomcap` | — | — | kaggle_geomcap: 610 |
| **D91** | A LEAD, NOT A FINDING, AND THE MOST CONSEQUENTIAL ONE THE PROJECT HAS: correctness may be decodable from the orbit's shape in the TRANSIENT and nowher | — | — | `run_h0_within.py` | — |
| **D92** | A STALE-CSV BUG SURFACED A REAL, REPRODUCIBLE, WEAK CORRECTNESS SIGNAL THAT D84's ORIGINAL TEST COULD NOT SEE: shape decodes correctness at 61.0% bala | — | — | `run_correctness_decode.py`, `run_h0_within.py` | — |
| **D93** | D91's LEAD DOES NOT REPLICATE IN THE POWERED, PRE-REGISTERED TEST -- AND THE NULL IS TRUSTWORTHY BECAUSE THE INSTRUMENT'S OWN DETECTION FLOOR WAS ITSE | — | — | `run_h0_within.py` | — |
| **D94** | H3 IS MEASURED, FOR THE FIRST TIME, WITH THE ESTIMATOR H3 IS ACTUALLY ABOUT -- AND THE MAP CONTRACTS UNIFORMLY: median rho = 0.8550 over 16 prompts, a | — | — | `run_h3_contraction.py` | — |
| **D95** 🔻 | THE FIRST CAUSAL EXPERIMENT RAN, ITS INSTRUMENT VALIDATED ITSELF, AND ITS OWN PRE-REGISTERED GATE DECLARES THE RESULT VOID -- NOT NULL. | `kaggle_patch` | — | — | kaggle_patch: 2 |
...
| **D99** | THE STATE IS CONFINED TO A SPHERE, AND THAT MAKES THE SETTLE/LOOP/DRIFT TRICHOTOMY EXHAUSTIVE RATHER THAN A LIST OF THREE GUESSES. | `ds_pertoken`, `kaggle_h0bank` | — | — | ds_pertoken: 1, kaggle_h0bank: 547 |
...
| **D103** | EVERY ACCURACY NUMBER IN THIS PROJECT IS AN ORACLE OVER DEPTH, AND IT INFLATES BY 2x. | `ds_grid` | — | — | ds_grid: 1 |
...
| **D105** 🔻 | CLRS-TEXT, THE ONE TASK SOURCE INSIDE HUGINN'S OWN TRAINING MIXTURE, SCORES 0.0% EXACT AND 14.9% CONTAINMENT AT DEPTH 4 -- BUT THE RUN IS NOT YET EVID | — | — | — | — |
...
| **D117** | D85's GEOMETRY-CAPABILITY NULL NOW COVERS THE DYNAMICS, NOT ONLY THE SHAPE -- AND THE SAME RUN REPRODUCES PROMPT LENGTH AS THE ONLY LIVE PREDICTOR. | — | — | — | — |
...
| **D120** 🔻 | TRAINING PUSHES THE RECURRENT MAP TOWARD THE EDGE OF STABILITY, AND IT IS DONE BY THE EARLIEST CHECKPOINT AVAILABLE. | `kaggle_rho_ckpt_` ⚠MISSING | — | `preflight.py` | — |
...
| **D124** 🔻 | THE CANCELLED CLRS RUN'S OUTPUT SURVIVED, AND READING IT RETRACTS HALF OF D105 AND SHOWS THE RUN NEVER IMPLEMENTED DR1's SPECIFICATION. | `kaggle_clrs` | — | — | kaggle_clrs: 2 |
...
- **D34** — SUPERSEDED BY D50 (2026-08-04): the direction is the current-token contrast and registerr is a window artefact *(evidence column: Kaggle T4 kernel arsen4ikvar/geometry-position-states (raw per-positio)*
...
- **D44** — D42 CONFIRMED ON THE OPERATOR: the untrained map contracts at ρ=0.7150±0.0069, the trained at ρ=0.8866±0.0362. *(evidence column: Kaggle T4 geometry-rho-direct, float32, 12 prompts × 2 orbits × 128 un)*
...
- **D51** — SUPERSEDED BY D52 (same day): partial, 4 of 8 checkpoints and n=1 untrained. *(evidence column: Kaggle T4 geometry-rho-ckpt-a/b, 4 intermediate checkpoints, same code)*
- **D52** — D44's pseudoreplication is CLOSED on both sides. *(evidence column: Kaggle T4 geometry-rho-ckpt-a/b/c/d (8 checkpoints) + geometry-rho-see)*
...
- **D57** — The Caesar screen is 0% in every cell — but the trained model answers almost every prompt with the memorised p *(evidence column: Kaggle T4 geometry-caesar-screen (6 cells × 20 items × 2 arms); tokeni)*
- **D59** — ρ IS STEERABLE by a gradient-free static rescale — paired slope +0.2853 ± 0.0450, r=+0.913, p=2.3e-04, 9/12 pr *(evidence column: Kaggle T4 geometry-eps-sweep; attn.proj and mlp.proj of all 4 core blo)*
- **D60** — Prompt format matters enormously — a trivial copy task goes 15% → 100% under Huginn's own chat template — but  *(evidence column: Kaggle T4 geometry-chatfmt; identical items under 3 prompt formats, tr)*
- **D61** — When Huginn cannot do a task it does not fail randomly — it RETRIEVES something adjacent, by one of at least s *(evidence column: Qualitative taxonomy over the ~27 logged outputs of geometry-caesar-sc)*
- **D63** — The attn-vs-mlp split is UNDERPOWERED and its own pre-registered gate says do not interpret it: the reproducti *(evidence column: Kaggle T4 geometry-eps-split; 12 prompts, paired, 5 ε values per arm)*
...
- **D77** — HOW FAR bf16 STORAGE SUPPORTS TRAJECTORY GEOMETRY, MEASURED RATHER THAN ASSUMED: exactly to the unroll where t *(evidence column: Controlled comparison with zero GPU: the SAME fp32 orbits from geom-ba)*
- **D78** — THE PER-UNROLL RANK READOUT HAS AN UNCONTROLLED SOURCE OF RUN-TO-RUN VARIANCE: initializestate draws h0 from a *(evidence column: delta log10 rank)*
- **D105** — CLRS-TEXT, THE ONE TASK SOURCE INSIDE HUGINN'S OWN TRAINING MIXTURE, SCORES 0.0% EXACT AND 14.9% CONTAINMENT A *(evidence column: Kaggle T4 geometry-clrs; the number is read from the run's own log lin)*
