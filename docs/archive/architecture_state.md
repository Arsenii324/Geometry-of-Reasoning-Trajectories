...
| `run_depth_profile.py` | **D92-D94 — does depth improve the answer, and is `correct` a property of the model? 0-GPU over two banks already on disk.** Three arms. (1) The project's `correct` label is `min_r rank_r == 1`, a MINIMUM OVER UNROLLS; it is scored against the best any FIXED depth attains, which is the only thing a user can select. (2) Accuracy against depth, tested family-paired with an exact sign test — the FAMILY is the unit, because 504 orbits are 21 clusters of 24 from one generator each and pooling them would read cluster size as evidence; the peak unroll is a module constant fixed a priori from the pooled curve, never re-chosen per family. (3) Whether the answer is lost or relocated, from the generation bank, against a LENGTH-MATCHED DECOY — the gold of a different item in the same family — because outputs grow 1.45x longer with depth and a short gold string is contained more often for free. The refuted 'conversational framing' mechanism is computed and reported alongside, not deleted, because a reader will propose it. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced.)* | yes | `depth_profile.csv` |
