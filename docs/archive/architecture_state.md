...
| `run_geomcap.py` | **does the orbit's SHAPE track what the model can compute, 0-GPU over the `geometry-geomcap` bank.** Correlates four geometric statistics against per-family capability over 21 families, and — this is the part that makes a null readable — against the reliability ceiling the same run measures. D78's unseeded `h_0` means ten forwards of one prompt differ in the initial condition and nothing else, so `analysis.reliability` converts that spread into the correlation a PERFECT relation would have shown; an observed rho is quoted against that band, never bare. Prompt length is the pre-registered positive control, and every correlation is REFUSED rather than reported when its column has no spread (scipy returns NaN, and `NaN < alpha` is False, so an undefined correlation would otherwise be filed as a null). *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced by `tests/test_architecture_consistency.py`.)* | yes | `geomcap.csv`, `geomcap_summary.csv` |
| `run_window_law.py` | **D80 — the shape statistics are mostly a CLOCK, 0-GPU over every banked trained orbit.** Slides a FIXED-WIDTH window along the unroll axis (fixed width because participation ratio grows with sample count, and D74(6) records that confound as total at rho = -1.000) and compares how much each statistic moves along depth against how much it moves across prompts. Paired within each orbit over the depth range every orbit reaches, so neither the mix of banks — ds_bank runs to 128 unrolls, b6bank and geomcap to 64 — nor any per-prompt offset can manufacture the effect. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced by `tests/test_architecture_consistency.py`.)* | yes | `window_law.csv` |
| `run_surrogate.py` | **is the orbit's geometry anything more than a linear contraction from a random start? 0-GPU over the 7 prompts that have BOTH an Arnoldi spectrum (`ds_eigen`) and a banked orbit (`ds_bank`).** A sufficiency test rather than another null: a surrogate built from the measured eigenvalues and a random initial condition knows no prompt, task or answer, so where it matches, the real orbit's shape carries nothing else. Three-way against a swapped-spectrum surrogate and an isotropic null, with `spectrum_tail` measuring sensitivity to Arnoldi's 8-mode truncation. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced.)* | yes | `surrogate.csv` |
| `run_linearity.py` | **D82 — is one unroll a linear map on the states the orbit visits? 0-GPU over every banked trained orbit.** Held-out one-step prediction of the step directions, fitted in the training steps' own SVD basis and scored on steps the fit never saw (in-sample is meaningless: ~90 steps in 5280 dimensions fit exactly by construction). The reported quantity is `r2_linear / in_basis` — of what the fitted subspace CAN express, how much the operator gets — because the absolute score is capped by the rank allowed, and reading it bare is how the first version of this analysis concluded the opposite. The scalar baseline is projected into the same subspace, so the contrast is operator-vs-rate rather than projected-vs-unprojected. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced.)* | yes | `linearity.csv` |
| `run_depth_accuracy.py` | **does the model SAY what its rank curve says it knows? 0-GPU over the `geometry-depthacc` kernel's banked strings.** Closes the gap D68(5) named itself — rank is not capability. Re-scores every decoded string locally rather than trusting the kernel's own field, so the scorer can change without another GPU hour (D60 measured an 85-point swing from a scoring/prompting decision). Reports exact match AND containment, because their gap is the discourse effect in units of accuracy; tests depth paired over families with an exact sign test, since h_0 is unseeded (D78) and an unpaired comparison would carry ~8 points of noise for free. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced.)* | yes | `depth_accuracy.csv` |
| `run_shape_decode.py` | **can ANY function of the orbit's SHAPE decode the task, or only its position? 0-GPU over the banked trained orbits.** Answers D79(5)'s stated limit — "a geometric statistic nobody has thought of could still differ" — by replacing hand-picked statistics with a cross-validated classifier over the Gram matrix of unit step directions, which is a complete invariant of the path under translation, rotation and scaling (verified on real orbits at max|delta| = 1.5e-15). Raw states are run as the POSITIVE CONTROL at exactly the same feature count, because Huginn re-injects the prompt at every unroll (D70(4)) and a null on shape means nothing unless the same classifier succeeds on position. The null is a label permutation, not an assumed chance level. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced.)* | yes | `shape_decode.csv` |
| `depth_profile.csv` | **The corrected depth/accuracy profile**, recovered with `run_depth_profile.py` from the unmerged branch. Scores containment after separating the leaked role marker that D145 shows corrupts both D86's and D133's numbers. *(Recovered 2026-08-10.)* |
| `run_correctness_decode.py` | **is correctness decodable from the orbit's SHAPE, and what could this design have seen? 0-GPU.** Bounds D84's inconclusive correctness arm. Features are centred within (family, gold) STRATUM and the permutation null permutes within stratum, because family centring alone leaves D72's confound intact — a first version scored 59.6% at p = 0.0025 carried by `sub1` at 86.2%, where only 4 of 24 items sit in a gold value carrying both classes, so correctness there is nearly a function of the answer and the classifier reads it off the prompt (which D84 shows it can do at 100%). Centring is fitted inside the CV fold, not on the full data. A detection floor is measured by planting effects of known size on WITHIN-STRATUM-PERMUTED labels, so the curve reports the design's reach rather than the signal already present. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced.)* | yes | `correctness_decode.csv` |
| `run_h0_within.py` | **PRE-REGISTERED, written before its data existed.** Does the shape predict correctness WITHIN a prompt, where the only thing varying is the unseeded h_0 (D90)? This is the design the contradicting literature uses — it gets within-prompt variance from stochastic rollouts, Huginn's decode is deterministic, and h_0 supplies the same handle. Fixes in advance: the 8-window grid (identical to D91's), the statistic, the within-prompt permutation null, the Bonferroni threshold, and a decision rule that requires a transient window to clear alpha AND the far tail not to. Carries a timing control, because `best_depth` varies 7-25 within one prompt (D90) and correct/incorrect orbits could differ in WHEN they peak rather than in shape. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced.)* | yes | `h0_within.csv` |
| `run_h3_contraction.py` | **Measures H3's actual quantity for the first time.** `contraction_from_pair` needs two orbits of the SAME prompt from DIFFERENT h_0; no bank satisfied that at depth until `geometry-h0bank` (D93) incidentally produced 16 prompts x 32 unseeded draws. Fits the per-step contraction multiplier rho over ~960 orbit pairs and reports the spread, the share of pairs at rho >= 1 (the loop/drift signature the supervisor asked about), and the residual gap floor. Predictions written into the docstring before running, per CLAUDE.md section 1. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced.)* | yes | `h3_contraction.csv` |
| `run_qk_analysis.py` | **G2, the supervisor's own open item, measured at last.** Per-unroll cosine(q,k) at the answer position on the D88 marker control (A and C select the SAME computation, B a different one, one character apart at identical token count). Rejects the pre-registered 'QK just reads the token' prediction, but its built-in confound check shows the gap tracks the RANK gap (Spearman +0.750), so 'tracks the computation' is NOT established -- the script cannot print the optimistic verdict without also printing the confound. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced.)* | yes | `qk_probe.csv` |
| `run_census_analysis.py` | **The task-supply asset, and a corrected estimator.** `geometry-census` screens 21 families x 12 items x 4 h_0 draws then deepens the splitting/boundary items by 20 more; this recovers the per-item observability table every causal design now selects from, after D47 and D95 both died on task supply. The winner's-curse rule first written into directions.md E3 -- 'use stage-2 draws only' -- was CHECKED BY SIMULATION AND REFUTED (pooled wins on bias -0.0014 vs -0.0180 and on error 0.0698 vs 0.0778), so pooled is primary and the gap is printed. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced.)* | yes | `census.csv` |
| `run_rho_vs_behaviour.py` | **The first test putting a geometric and a behavioural quantity on the SAME prompts**, at zero GPU: B1c measures the causal contraction rate per recipient, B4 measured `best_depth` per problem, and all 16 of B1c's recipients exist in B4's 36-problem set. Prediction is written in the docstring before the join runs, with the reason a NULL is the likelier outcome stated in advance (D112 measures the answer as top-ranked at unroll 4 while 87% of the state's journey remains). Detection floor simulated through the same code path: 5% false positive at zero effect, 98% detection of a rho-spread of 0.024 -- three times smaller than the spread already observed. *(Added 2026-08-09; doc otherwise FROZEN, inventory still enforced.)* | yes | stdout only |
| `run_addk_arms.py` | **D110's three identification arms, made re-derivable.** A provenance audit flagged the answer-token arm as a mismatch and observed, correctly, that no script referenced `ds_addk` — the analysis lived only in shell heredocs. Re-running reproduces all three arms exactly (-0.4810 / -0.5265 / +0.0719 against -0.481 / -0.527 / +0.072), so the audit's mismatch was a false positive from a different minimum-stratum rule; this module requires a stratum to span >= 3 levels. Keys problems on (level, gold) because `_addk`'s item slots collide at high k (D110(b)). *(Added 2026-08-10; doc otherwise FROZEN, inventory still enforced.)* | yes | stdout only |
| `run_embsep_nonlinear.py` | **D136: can any simple rule read the rotation regime off the token embedding?** Six leave-one-out classifiers (cosine-kNN k=1/3/5, ridge lambda=1/100, an ||emb|| threshold) on A23's 49 deterministic nouns. All six sit at or below the 0.592 majority baseline. The primary statistic is best-of-six against the null distribution of best-of-six, recomputing every classifier on all 2000 permuted draws -- 0.551 vs 0.589, p = 0.8081 -- because a single-classifier null would have scored the same number at p = 0.347. Closes D134's untested "by elimination it lives in the embedding" inference in both its linear and nonlinear forms. *(Added 2026-08-10.)* | yes | stdout only |
| `run_rotation_continuum.py` | **D137: is the regime a thresholded continuum or two distinct dynamical regimes?** Applies `traj_geom.metrics.dynamics.rotation_power` to all 204 banked A23 orbits. Passes its own instrument null (rotating median R 0.896 vs settling 0.336, p = 0.0002), places the 8 "mixed" orbits at the boundary (p = 0.0008) and ~~finds the distribution GAPPED (p = 0.0002)~~ **[the gap result is RETRACTED by D141 -- it does not separate the regimes. The surviving statistic is a single threshold, R > 0.6677, reproducing the binary label on 691/696 orbits across the three banks]**. Its raw rows then split the two mixed nouns apart: `word`'s rotating orbit has the LOWEST period-6 power of its four (a detector false positive) while `set`'s has the highest -- so 50 of 51 nouns are deterministic and D135's "token AND sequence" reading is corrected. *(Added 2026-08-10.)* | yes | stdout only | **Extended 2026-08-10 (D138): takes a BANK argument** (`embsep` / `nounsweep` / `wordswap`) so the same statistic runs on orbits banked for other experiments; the embsep control reproduces D137 digit-for-digit. P2/P4 now report NOT EVALUATED when a bank has no mixed nouns -- the first nounsweep run printed `p = 0.0002 -> AT THE BOUNDARY` over an empty set, because `(0+1)/(N+1)` against a nan observed value reads as maximally significant.
| `run_einterp_analysis.py` | **A24: where along `e` does the regime flip, and how sharply?** Reads the interpolation curves banked by `scratch/ds_einterp/`. P1 is a no-op-patch identity check that VOIDS the run on mismatch; P1' (does the attractor follow `e` from a foreign initial state) is reported separately because its failure is a finding about the model, not a broken instrument. P2 reports the 10-90% transition width by linear interpolation rather than a sigmoid fit -- with 21 grid points a fitted width is dominated by the two points straddling a jump and claims resolution the grid lacks. P3, the within-regime control, is load-bearing: without it "any `e` perturbation moves the dynamics" explains P2 equally well. **P1 turned out UNEVALUABLE (D140): it demanded 1e-6 agreement between two forwards, and D78 records that h_0 is drawn from an unseeded RNG no kernel seeds -- so the script now measures the h_0 floor from carriers computed unpatched more than once, reports P1 rather than gating on it, and leans on P3 for calibration. P2/P3 are read by THRESHOLD CROSSING rather than R span; the span version reported the within-regime control as failing when one of its two pairs never leaves the regime at all.** *(Added 2026-08-10.)* | yes | stdout only |
| `run_regimebehav_analysis.py` | **A26: does crossing the regime boundary change what the model DOES?** Analyses the regression-discontinuity run -- two grid points 0.05 apart in `e`-space on opposite sides of the boundary, same prompt, same required answer. P1 checks the no-op patch reproduces the RANK CURVE exactly (a different code path from A25's state check, never previously verified under patching) and VOIDS on failure. P5 is a readout positive control: if the gold rank does not move across the full sweep, a null at the boundary is uninterpretable and the script says so rather than reporting it. **P4 is the reason the file exists** -- it plants effects of known size into the observed paired differences and reports the fraction this design catches, which is exactly what D132's identical comparison lacked. *(Added 2026-08-10.)* | yes | stdout only |
| `run_statetrack_analysis.py` | **A27: can Huginn state-track, and does depth rise with difficulty?** Enforces the gate ORDER rather than suggesting it -- P1 (capability) gates P2 (dynamic range) gates P3 (H2), because this project has twice computed geometry on an axis the model could not do (D118 VOID at 0.0%, D100 withdrawn). P1 uses an exact binomial against the level's MAJORITY-CLASS baseline, not the 1/3 chance floor, so a constant responder cannot pass. P3 excludes m = 0 a priori -- D143's headline was entirely one trivial level and this is the same trap. P5 probes whether accuracy is explained by the last swap rather than by difficulty, which would mean pattern-matching rather than tracking. *(Added 2026-08-10.)* | yes | stdout only |
| `run_depth_profile.py` | **RECOVERED 2026-08-10 from the unmerged branch `claude/geometry-remote-continued` (commit 92600a2), which had been sitting 2 commits ahead of a 160-commit HEAD since 2026-08-09.** Regenerates the depth/accuracy profile from banked data with no GPU, and -- the reason it matters -- it scores containment with the leaked role marker SEPARATED, which is the defect D145 found in both D86's and D133's scorers. Ships `results/depth_profile.csv`, `docs/depth_profile.md` and `tests/test_depth_profile.py` (11 tests, all passing on HEAD, including `test_paired_test_unit_is_the_family_not_the_orbit` and `test_containment_matches_whole_tokens_only`). Merged BY FILE, not by `git merge`: the branch's ledger/PLAN edits were dropped because they would collide with 160 commits of later work, and its second commit `44255b2` was dropped as provably redundant (`shape_decode.csv` byte-identical on HEAD). | yes | `results/depth_profile.csv` |
| `runs_status.py` | **Operational, not analytical.** One command for every job on both platforms from `scratch/RUNS.json`, and with `--pull` it downloads each finished DataSphere job and reads its RAW logs for the failure signatures this project has actually hit (`Killed` = host OOM, `OutOfMemoryError` = GPU OOM, the traj_geom import mismatch, SELF-CHECK FAILED, VOID). Exists because status fields lied three times in one day: an ERROR job that was fully recoverable, a SUCCESS job that printed a verdict over zero data, and a host-OOM misread as a network error. *(Added 2026-08-09; doc otherwise FROZEN.)* | no | none |
...
| `run_cycle_residual_decode.py` | A3: does the learned block cycle carry TASK identity? Decodes the A/B marker from the per-block last-step residual on `ds_blockbank`'s paired orbits (65 sequences × 2 markers, inputs identical but for one token), folds grouped by sequence, PCA(40) in-pipeline, plus a depth sweep that refutes the re-injection deflation. 0-GPU | yes | `cycle_residual_decode.csv`. Basis of D205. |
