...
\subsection{E1: PARARULE-Plus вЂ” Winding Does Not Track Depth}
...
\subsection{E2: Synthetic Counting вЂ” H3 Support}

Running $\pm1$ sum, length $n_{\text{ops}} \in \{2\ldots 32\}$, 5 seeds. All trajectories settle, but:

\begin{itemize}
    \item Steps-to-settle $\sim n_{\text{ops}}$: $\rho = 0.718$, $p < 1e-5$ вЂ” effective computation grows with state length.
...
\subsection{E3: Length-Matched Control вЂ” The Killer Experiment}
...
3 & $5.84 \pm 0.01$ & $-0.498 \pm 0.

> Gera Drobierro:
} Each token's latent path falls into one of several regimesвЂ”settling, looping, or driftingвЂ”distinguishable via Lyapunov exponents, recurrence, and persistent homology signatures.
...
\begin{itemize}
    ...
    \item \textbf{Methodological contribution.} Step normalization is a critical condition for clean orthogonality measurementвЂ”a factor not considered in the original paper.
\end{itemize}
...
> Gera Drobierro:
рџ–ј \cite{grazzi2024unlocking} prove that positive-eigenvalue LRNNs cannot solve parity. Huginn's contractive dynamics place it in this failure class.
...
Tulchinskii, E., et al. (2025). "Quantifying Logical Consistency in Transformers via QueryвЂ“Key Alignment." \textit{EMNLP 2025}. arXiv:2502.17017.
