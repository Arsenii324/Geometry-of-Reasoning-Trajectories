...
| G12 | **What the depth is FOR: readout.** Decoding the total from the ANSWER TOKEN rises R²=0.675→0.993 over ~24 unrolls (rho=+0.983, all p<0.001) while the per-position register is flat. Same model, same depths — the difference is *where* the state is read. 220 prompts all at 74 tokens. | D38 |
