...
| `geometry-lenmatch` | Kaggle T4 | **the test D84 demands** — two different computations at an IDENTICAL token count, selected by one trailing marker, so task identity is not collinear with prompt length | RUNNING (3 pairs x 32 items x 2 markers, per-item length gate enforced in-kernel before the model loads) |
