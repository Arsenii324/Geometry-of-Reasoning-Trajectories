"""A20: does ONE WORD flip the recurrent map between settling and rotating?

D129 found three templates whose rotation status tracks the instruction wording rather
than anything else measurable:

    "report the largest SYMBOL of the sequence"   -> rotates 32/32  (lenmatch)
    "report the largest SYMBOL of the sequence"   -> rotates 132/132 (A19)
    "report the largest ELEMENT of the sequence"  -> rotates   0/22  (blockbank)

A character-exact diff of the A19 and blockbank templates returns ONLY `element` ->
`symbol`, twice. Token count is 53 in both, the task is identical, the sequence length
is identical, and A19 rotates at every symbol-diversity level from 2 to 7 -- so neither
length, task, nor content explains it.

**But the substitution has never been varied inside one run.** `element` has been seen
in one job and `symbol` in two, so the comparison is across runs and inherits whatever
else differs between them. This job removes that: the SAME sequences, the SAME model
call, both wordings, interleaved.

PRE-REGISTERED.
  P1  PRIMARY. Rotation fraction differs between the two wordings on IDENTICAL
      sequences. D129 predicts ~100% for `symbol` and ~0% for `element`.
  P2  IF BOTH WORDINGS ROTATE EQUALLY, the D129 association was an artefact of
      something else differing between those runs, and the wording lead is dead. That
      is a clean answer and the row will say so.
  P3  TOKEN GATE. `element` and `symbol` must give the SAME token count, else the
      contrast is a length contrast. Checked in-kernel per pair; if they differ the
      run reports the difference and the result is read as length-confounded.
  P4  PAIRING. Every sequence appears under BOTH wordings, so the comparison is
      within-sequence and the unit is the sequence.
  P5  A THIRD WORDING, `item`, is included as a tiebreaker: if `symbol` and `item`
      both rotate and `element` does not, the effect is not a two-word coincidence.
"""
...
NUM_STEPS = 64
WORDS = ("symbol", "element", "item")   # THE variable; everything else is fixed
N_SEQ = 18                              # 18 seqs x 3 words x 2 markers = 108 orbits
