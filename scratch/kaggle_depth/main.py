...
def main():
    ...
    for n_ops in N_OPS:
        for seed in range(N_SEEDS):
            try:
                ...
                m = lg - ld
                r = {"n_ops": n_ops, "seed": seed, "answer": a, "distractor": dis,
    ...
    if solved:
        ...
        if len(by) > 2:
            ks = sorted(by); vs = [np.mean(by[k]) for k in ks]
            rho, p = spearmanr(ks, vs)
            print(f"\n  THE TEST -- rho(r*, n_ops) = {rho:+.3f}, p = {p:.4f}, N={len(ks)} levels")
            print(f"  VERDICT: {'depth scales with difficulty' if p < 0.05 and rho > 0 else 'no evidence that required depth scales with difficulty'}")
    else:
