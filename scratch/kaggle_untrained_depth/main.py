...
N_SEEDS = 44   # unchanged, so the curve is directly comparable to the trained run
...
def main():
    ...
    # UNTRAINED: identical architecture/config/tokenizer/RMSNorm, random weights.
    # D40 showed the untrained model reproduces the ENDPOINT (r=32) of every
    # positive finding. It did NOT test the depth TRAJECTORY, which is what
    # D37/D38 actually claim. If the untrained readout curve also rises
    # 0.65 -> 0.99 with depth, those claims are architectural too; if it is flat
    # at ~1.0 from r=1, then the RISE is a trained property even though the
    # endpoint is not.
    from transformers import AutoConfig
    cfg = AutoConfig.from_pretrained(MODEL_ID, revision=REVISION, trust_remote_code=True)
    torch.manual_seed(0)
    model = AutoModelForCausalLM.from_config(cfg, trust_remote_code=True).to(
        torch.float32).to("cuda").eval()
    print("UNTRAINED model instantiated (random weights)", flush=True)
