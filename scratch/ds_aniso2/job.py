"""A33: the corrected A31. Three groups of directions, not a correlation over random rays.
...
def main():
    ...
    t0, rows = time.time(), []
    # THREE GROUPS, not a correlation over random rays. D156: random directions in
    # 279,840 dimensions are all orthogonal to a 6-dim span (expected alignment
    # sqrt(6/D) = 0.0046, observed sd 0.0013), so a correlation had no variable to
    # correlate with. Sampling INSIDE each span gives maximal contrast instead.
    def draw(group):
        if group == "random":
            u = torch.randn(e0.shape, generator=g, device=e0.device, dtype=e0.dtype)
        else:
            Q = Qr if group == "in-rot-span" else Qs
            c = torch.randn(Q.shape[1], generator=g, device=Q.device, dtype=Q.dtype)
            u = (Q @ c).view_as(e0).to(e0.dtype)
        return u / u.norm()

    plan = [("in-rot-span", k) for k in range(N_DIR // 3)] + \
           [("in-set-span", k) for k in range(N_DIR // 3)] + \
           [("random", k) for k in range(N_DIR - 2 * (N_DIR // 3))]
    for group, k in plan:
        u = draw(group)
        uf = u.flatten().float()
        ...
        rec = {"dir": k, "group": group, "align_rot": a_rot, "align_set": a_set,
        ...
        if (far > THRESHOLD) == (r_patched > THRESHOLD):
            ...
            print(f"  [{group:11s}] dir{k:2d} a_rot={a_rot:.4f} a_set={a_set:.4f}  CENSORED", flush=True)
        else:
            lo, hi = 0.0, max_dist
            ...
            print(f"  [{group:11s}] dir{k:2d} a_rot={a_rot:.4f} a_set={a_set:.4f}  d={d / chord:.3f} chords "
